const lzma_api = (function () {
  ...
  function $init(this$static, input, output, length_0, mode) {
      ...
      encoder._writeEndMark = typeof this?.LZMA?.disableEndMark == "undefined";
          ...
          onmessage = function (e) {
              if (e && e.data) {
                  ...
                  if (e.data.action == action_decompress) {
                      //@ts-ignore
                      this.LZMA.decompress(e.data.data, e.data.cbn);
                  } else if (e.data.action == action_compress) {
                      //@ts-ignore
                      this.LZMA.compress(e.data.data, e.data.mode, e.data.cbn);
                  }
                  /** xe */
                  /// co:if (e.data.action == action_compress) {
                  /// co:    this.LZMA.compress(e.data.data, e.data.mode, e.data.cbn);
                  /// co:}
                  /// do:if (e.data.action == action_decompress) {
                  /// do:    this.LZMA.decompress(e.data.data, e.data.cbn);
                  /// do:}
              }
          };
      }());
...
const LZMA_API = lzma_api;
export const {compress,decompress} = LZMA_API;
export default LZMA_API;
